...
class HeaderProfileCollectionReusableView: UICollectionReusableView, UITextViewDelegate {
    ...
    func updateView() {
        ...
        if user?.id == Api.Userr.CURRENT_USER?.uid {
            ...
            editButton.addTarget(self, action: #selector(self.goToSettingVC), for: UIControl.Event.touchUpInside)
        } else {
    ...
    // Opens website URL
    func makeURLOpen() {
        let attributedString = NSMutableAttributedString(string: (user?.website)!, attributes:[NSAttributedString.Key.link: URL(string: (user?.website)!)!])
