...
class TravelViewController: UIViewController {
    ...
    private func setupTableView() {
        ...
        if #available(iOS 10.0, *) {
            travelTableView.refreshControl = refreshControl
        } else {
            travelTableView.addSubview(refreshControl)
        }
        
        
        // Configure Refresh Control
        refreshControl.addTarget(self, action: #selector(refreshData(_:)), for: .valueChanged)
        travelTableView.estimatedRowHeight = 521
        travelTableView.rowHeight = UITableView.automaticDimension
        travelTableView.dataSource = self
        travelTableView.allowsSelection = true
    }
    ...
    private func updateView() {
        ...
        travelTableView.isHidden = !hasPosts
    ...
    func loadUserPosts() {
        Api.HashTag.observeTravel { (post) in
...
// MARK: Segue Actions
extension TravelViewController: DetailTopicDelegate {
