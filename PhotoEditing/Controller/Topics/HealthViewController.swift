...
class HealthViewController: UIViewController {
    ...
    @IBOutlet weak var health: UITableView!
    ...
    private func setupTableView() {
        ...
        if #available(iOS 10.0, *) {
            health.refreshControl = refreshControl
        } else {
            health.addSubview(refreshControl)
        }
        
        
        // Configure Refresh Control
        refreshControl.addTarget(self, action: #selector(refreshData(_:)), for: .valueChanged)
        health.estimatedRowHeight = 521
        health.rowHeight = UITableView.automaticDimension
        health.dataSource = self
        health.allowsSelection = true
    }
...
// MARK: Segue Actions
extension HealthViewController: DetailTopicDelegate {
