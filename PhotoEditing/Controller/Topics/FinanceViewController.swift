...
class FinanceViewController: UIViewController {
    ...
    @IBOutlet weak var finance: UITableView!
    ...
    private func setupTableView() {
        ...
        if #available(iOS 10.0, *) {
            finance.refreshControl = refreshControl
        } else {
            finance.addSubview(refreshControl)
        }
        
        
        // Configure Refresh Control
        refreshControl.addTarget(self, action: #selector(refreshData(_:)), for: .valueChanged)
        finance.estimatedRowHeight = 521
        finance.rowHeight = UITableView.automaticDimension
        finance.dataSource = self
        finance.allowsSelection = true
    }
...
// MARK: Segue Actions
extension FinanceViewController: DetailTopicDelegate {
